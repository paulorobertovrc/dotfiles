...
- **Modelo + esforço por passo (quando o projeto pedir rigor):** antes de cada resposta substantiva,
  recomendar modelo (Opus/Sonnet/Haiku) e nível de esforço (escala 1–5). Se o recomendado divergir do
  ativo, **PARAR** e aguardar a troca — nunca trocar sozinho, seja *upgrade* ou *downgrade*. Rubrica:
  modelo forte para design/arquitetura/correção crítica/raciocínio difícil; leve para implementação
  padrão a partir de design travado.
